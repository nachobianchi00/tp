...
async function update (req: Request, res: Response) {
    const character = await repository.update(req.params.id as string, req.body.sanitizedInput)
    if(!character) {return res.status(404).send({message: 'character not found'})}

    return res.status(200).send({message: 'Character updated', data: character})
